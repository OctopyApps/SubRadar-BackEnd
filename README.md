...
и `dev` (`.github/workflows/backend-tests.yml`), вместе с
`tests/check_new_code_has_tests.sh` — проверкой, что у файлов, изменённых
в PR под `internal/`, есть тесты рядом (хотя бы один `*_test.go` в той же
директории).
